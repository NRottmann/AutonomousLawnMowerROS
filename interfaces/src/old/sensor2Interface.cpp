...
{	
	...
	msg_out.r2 = reinterpret_cast<uint8_t>((unsigned char)msgRight.data.c_str()[0]);
	msg_out.r1 = reinterpret_cast<uint8_t>((unsigned char)msgRight.data.c_str()[1]);
	msg_out.l1 = reinterpret_cast<uint8_t>((unsigned char)msgLeft.data.c_str()[0]);
	msg_out.l2 = reinterpret_cast<uint8_t>((unsigned char)msgLeft.data.c_str()[1]);
	return msg_out;
}
...
int main (int argc, char** argv){
	...
	// define publisher
    ros::Publisher sensor_pub = nh.advertise<interfaces::Sensor>("sensor2Data", 1000);
	// Try to initialize the right serial connection and catch errors
    try
    {
        serRight.setPort("/dev/sensor04");
    ...
    while(ros::ok()){
		// TODO: Sensors as ROS nodes directly onto the Arduino?
