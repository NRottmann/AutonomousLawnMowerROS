...
namespace gazebo
{
  class RobotControl : public ModelPlugin
  {
	...
	// old link orientations
	private: double oriLeftOld;
    ...
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
    {
	  ...
	  if (_sdf->HasElement("torque")) {
	    ...
	    ROS_WARN("odoemtry_plugin: Default torque is %f", torque);
	  }
	  ...
	  {
		radius = 0.1075;
	    ROS_WARN("odoemtry_plugin: Default radius is %f", radius);
	  }
	  ...
	  {
		axisDistance = 0.1075;
	    ROS_WARN("odoemtry_plugin: Default axisDistance is %f", axisDistance);
	  }
